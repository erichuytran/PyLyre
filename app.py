...
def tracks_page():
    ...
    return render_template("tracks_page.html", track=tracks, id_user=id_user)
...
def tracks_page():
    ...
    if liked == False:
        ...
        sql = """ SELECT * FROM tracks INNER JOIN artists ON tracks.id_artist = artists.id INNER JOIN albums ON tracks.id_alubm = albums.id INNER JOIN tracks_liked ON tracks.id = tracks_liked.id_track WHERE id_user = ? """
